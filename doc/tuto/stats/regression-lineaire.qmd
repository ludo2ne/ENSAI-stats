...
- `Std. Error` : l'erreur standard associée à chaque coefficient. Cela donne une idée de la précision des estimations
- `t value` :  utilisé pour tester l'hypothèse nulle selon laquelle le coefficient est égal à zéro (c'est-à-dire que la variable explicative n'a pas d'effet sur Y).
- `Pr(>|t|)` : La p-valeur associée à chaque test t. Une p-valeur faible (souvent inférieure à 0,05) indique que le coefficient est statistiquement significatif
